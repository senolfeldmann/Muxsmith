...
/// The shell's managed run-lifecycle state (D23): at most one run may be
/// active at a time. This is this task's own contribution to the shell's
/// managed state; other IPC surfaces (e.g. settings) add their own fields
/// at merge time.
#[derive(Default)]
pub struct AppState {
