...
**Date:** 2026-07-30 (session 30; Plan 11 EXECUTED AND CLOSED, pushed; Plan 12 next)
...
## Owner decisions waiting on his return

Nothing blocks the run. Both are recorded with their measurements.

1. **A one-key decision on the `cargo deny` guard.** Plan 11 leaves no permanent
   guard that the `unsound` scope key stays set. The review refuted the plan's
   ground for deferring it - `unused-ignored-advisory = "deny"` is one key in the
   same table, not new gate infrastructure, and the loss is not fully silent at
   defaults - and simultaneously recommended AGAINST setting it blind, because it
   also reddens the gate when an ignored advisory legitimately disappears
   upstream. Parked with the measurement attached; the ROADMAP Triggers entry
   carries it.
2. **Whether the house wants one `raw:` comparison vocabulary** across scoped and
   unscoped statements. Seven retained assertions keep their wording, two of them
   user-visible strings in two locales. Deliberately without a vehicle: picking
   one would pick the answer.
