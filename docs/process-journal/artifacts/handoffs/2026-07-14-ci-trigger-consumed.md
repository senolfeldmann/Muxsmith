<!-- Snapshot of HANDOFF.md at trigger consumption (CI green run 29320623887), 2026-07-14 (SI-5) -->
# Handoff
...
- The push-fired ROADMAP trigger is CONSUMED (2026-07-14): run 29320166456
  proved the T8 toolchain step works on-runner but failed windows-2025 on
  a REAL Windows-only defect (clippy result_large_err: Diagnostic crossed
  the 128-byte lint borderline there because Option<PathBuf> is 8 bytes
  larger on Windows; D36's claimants field moved it into range). Fixed
  a783a63 (deliberate commented allow at the two profile-load entry
  points; Diagnostic stays the uniform by-value error currency). Run
  29320623887 then went green on ALL legs; trigger line removed from the
  ROADMAP. Nothing blocks tagging from the CI side anymore.

## Next steps (priority order)

1. (done 2026-07-14: push + CI trigger consumed, see above)
