...
0. **DISCHARGED 2026-07-20.** The agent-side process-archive and research
   work that Şenol's 2026-07-17 directive placed before any new plan is
   done and its remaining pieces are parked by owner decision until after
   this product is finished. Product work is unblocked; the next session
   starts with the plan kickoff below. Specifics stay agent-side per the
   cross-project privacy line - do not look for them in this repo.
1. **Owner surface pass RESOLVED 2026-07-17**: all strings approved as
   shipped; the nav tab got its dedicated `nav-editor` key ("Editor", in
   gui-common beside its nav siblings - NOT in the 45-key editor catalog)
   via a four-eyes post-close fix (commit 2591cd4, pushed).
2. **Next-plan kickoff, now the first item of the next session** (Şenol's
   choice among 7, 8 and 9): Plan 7's
   anchor now carries the editor tooltips, curated-domain dropdowns,
   field-anchored validation markers, ordinal column, live locale switch,
   Fluent attribute reorg, help mode. Plans 8 and 9 unchanged.
