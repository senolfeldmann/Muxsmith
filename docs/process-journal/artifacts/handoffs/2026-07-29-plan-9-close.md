...
<!-- Provenance: snapshot of HANDOFF.md at the plan-9 close, session 26 (2026-07-29), per SI-5. -->
...
- **The gate is TEN parts** per BUILDING.md, foreground, no subsets, before any
  ...
  before every push as well and is not one of the ten. Two edits to that gate
  block are an open close action (below).
...
- master at `8feecd3`, clean. **7 commits unpushed** at the time of writing:
  the whole-branch fix wave, the close's house and ROADMAP writes, the SDD
  salvage and the journal entry. The session-close push runs the full gate
  first.
...
1. **The text-corrections pass, the one close action still open.** It is a
   product change, so it needs a dispatched implementer and a reviewer, and it
   is enumerated in the ROADMAP's Plan-9 close-action blocks: D102's unguarded
   scope boundary (its cheap producer is named), BatchView's else-branch
   comment and console.error string, the three overclaiming assertion messages
   in `dry_run_cli.rs` (the licence for the pre-existing one is carried by the
   close action), the delta-review wording sharpening on the `IdentifyCache`
   doc, the docs-accuracy two-edit item, and the two BUILDING.md gate edits
   (ledger-lint as an eleventh part, `--document-private-items` on the doc step
   at BOTH consuming sites). **Prerequisite for the last one:** two pre-existing
   ambiguous intra-doc links in `src-tauri/src/lib.rs` must be fixed in the same
   change, or the first gate run under the new flag goes red.
2. **Pre-1.0 gates** per the ROADMAP, once the pass lands.
3. **Archive duty:** session 26 is archived by the NEXT session; session 25 was
   archived at the start of this one.

## Open questions / risks

- **Two owner questions, both raised at this close and neither blocking.**
  Whether to re-fence the D49 experiment at the applier site - the Task-7
  reviewer measured that mutation and all three guards go red through their own
  assertions, so the question is answerable for the cost of one amendment and
  one measurement task; and what to do with the registered trigger whose
  condition (the only-G3 branch) no re-run of the fenced protocol can now
  produce.
