...
### Task 4: plan-JSON serialization fix (ADR D40) - in-flight addition

Routed from the whole-branch review (finding 1) with owner approval
2026-07-15: `TitleAction::Set(String)`, `ChapterSource::External(PathBuf)`
and `PrimaryAttachments::Subset(Vec<u64>)` are internally-tagged serde
enums with non-map newtype payloads - unserializable at runtime, so any
`run`/`dry-run --json` whose plan instantiates them panics at
report/json.rs:44 after a successful mux (pre-existing defect; the Task-2
README recipe first exercised it). Scope: struct variants for the three
enums, report/json.rs unwrap hardening (no panic on plan serialization),
ADR D40 authored in the decisions file by the implementer, per-variant
round-trip tests, regression e2e driving the README recipe verbatim
through `dry-run --json` and `run`. Full requirements:
`.superpowers/sdd/plan-5.8/task-4-brief.md` (salvaged at close). Stream C
worktree (`.worktrees/plan58-c`), merged after review like the others;
the whole-branch review reruns on the post-merge state.
