...
1. **The condition is one named `computed` in `EditorView.vue`, `nothingOpenedOrCreated`, and both surfaces read it**: `!model.value && currentPath.value === null`. No other gate in the view reads a bare `!model`.
...
**What this leaves uncovered, stated rather than implied.** Absence check P1 pins "defined once" at Task 4's verification and is a one-shot run, not a standing guard: nothing in the tree turns red if a later round re-inlines a bare gate, and the honest catch for that is the diff review. The check also matches the exact spelling `v-if="!model`, so a gate written with a space after the bang would escape it; every gate in this file is written without one, which is what the pre-state run measures.
...
| W2-p | The pre-session condition exists in exactly one place | Task 4 Step 7's absence check P1, whose fire is its own measured pre-state | yes |
...
**Amendment 1 adds no file to this list and no path to the commit block below**, which is checkable rather than asserted: the gate condition and its two gates live in `src/views/EditorView.vue` and its case lives in `e2e/editor-undo-redo.spec.ts`, and both are already members. D112 adds no catalog string, so neither `.ftl` moves.
