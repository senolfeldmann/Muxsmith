...
- Modify: `e2e/editor-undo-redo.spec.ts` (**two named cases only, the ones this task's own guard fronts with a confirm** - "Open resets" and "A failed open clears rather than keeps"; Step 4b. No other case in that file is touched - amendment 1)
...
- [ ] **Step 4b (amendment 1): repair the two Task-4 cases this task's own guard fronts.** Step 2 puts a confirm in front of `pickAndOpen` whenever `dirty` is true, and two cases in `e2e/editor-undo-redo.spec.ts` build history - which makes `dirty` true under D108 decision 4's formula - and then perform a second Open through that funnel. **The affected set is exactly two, enumerated from Task 4's own steps by that criterion** (builds history, then opens again): **"Open resets"** and **"A failed open clears rather than keeps"**. Every other case in that file fails the criterion and is NOT touched: the six mutation-path cases open once and then mutate; granularity, truncation, the depth cap and U1 perform no second Open; "Save marks rather than clears" saves before it ends, so `dirty` is false and it opens no second time; and amendment 1's D112 case opens twice but never edits the model, so `dirty` is false at both clicks and the guard cannot fire.
  - **The repair, in each of the two:** between the Open click and the assertions that follow it, assert that `confirm-dialog` is visible, then click `confirm-dialog-confirm`.
  - **No existing assertion in either case is removed, weakened or reordered.** Both keep testing exactly what they were written to test, over an editor that is genuinely dirty - which is the state a real user reaches, and the reason the alternative repair (saving before the second Open so the guard never fires) was rejected: it would have swapped the case's own subject to dodge a mechanism.
  - **Task 4 does not pre-adapt them.** `ConfirmDialog` does not exist until this task's Step 1, so those two clicks would fail there. The task that introduces the guard repairs the cases its guard changes, which is the same shape Task 3 and Task 4 already use for the comment regions their own edits falsify.
  - **This produces no new observable and the acceptance map gains no row:** W4-a to W4-c already grade the guard. It keeps two existing producers alive.

- [ ] **Step 5: verification.** The full gate as `BUILDING.md` enumerates it, foreground, green - **and the two repaired cases in `e2e/editor-undo-redo.spec.ts` are part of it**, so a repair that missed one shows up as a red gate here rather than at the push. The `gui-editor.ftl` recount must be 54; recompute and correct the budget comments. `git diff --stat` covers exactly the six files in the Files list.
