...
**Provenance:** Every task cites its sweep-walkthrough decision number (#N) or residue round (R1/R3). Full context: docs/ROADMAP.md pre-1.0 gates section + the process-learnings distillate kept with the project's non-repo material. Decision numbers D32+ are free.
...
Wave 1 (parallel):      Stream A: T3 -> T4 -> T4.5 (executor)
...
Wave-1 streams are disjoint code regions (verified: executor/, planner+validate, cli/commands+report, tests+ci, planner suggestion section, new proptest files). Genuine collisions: T6/T7 share planner.rs (serial within stream B); T12 shares ci.yml with T2 (ordered after); T4.5 shares the executor/ region and src-tauri/src/run.rs neighborhood with T4 (serial within stream A - added 2026-07-11 at the execution go, Şenol's D35 vehicle decision).
...
- Spec coverage: every pre-1.0 gate from ROADMAP (state 2026-07-11) maps to a task: #1→T14, #2→T22, #3→T5, #4→T15/16, #5+#12+R1→T13/17/18, #6→T6, #7→T7, #8→T8, #9→T3, #10→T4, #11+#13+R1→T9, #14→T2, #15→T1, #16→T10, #17→T19/20/21, #18b→T12, group T→T11, D35 implementation→T4.5 (added at the 2026-07-11 execution go per Şenol's vehicle decision). NOT in this plan (deliberate): CSP policy, README, guide/blogs, packaging (Plan 6), Dependabot (near-1.0) - they are non-code gates with their own ROADMAP entries. (The log-pruning DECISION D35 also stayed outside; only its implementation rides as T4.5.)
