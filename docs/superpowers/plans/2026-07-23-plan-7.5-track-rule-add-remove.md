...
>
> **House deviation from the skill text:** progress NEVER enters this document. No box in this file is ever ticked; the checkbox syntax is structure, not a tracking surface. The tracker is `.superpowers/sdd/plan-7.5/progress.md` (git-ignored during execution; salvaged at the plan close to `docs/process-journal/artifacts/plan-7.5-sdd/progress.md`).
...
Subagent-driven development (`superpowers:subagent-driven-development`): a **fresh implementer subagent per task**, an **independent reviewer per task** grading against this plan and the design, and a **whole-branch review at the plan close** before the close actions. Progress lives in `.superpowers/sdd/plan-7.5/progress.md` (git-ignored during execution; salvaged at the plan close to `docs/process-journal/artifacts/plan-7.5-sdd/progress.md`). This statement is binding per the pre-execution gate; `feedback_use_specified_execution_method` applies.
...
| D70 (placement, order, testids, keyboard reachability) | Task 1 (the template block verbatim; Step 6's diff check); keyboard reachability additionally exercised by Task 2's case 9 (Enter activation outside help mode) |
...
| Section 7 (deliberately out of scope) | three carriers, one per bullet group: the zero-new-surface global constraint (undo/redo, insert-at-position/up-down/multi-select, core/validate changes); the tasks' Files lists (attachment-rule add/remove and the plan-7 non-goals - no task touches their surface); Task 4's verbatim transcription + its Step 5 check (the spec-8.2 column-order cosmetic: the amendment text deliberately preserves "order, source, match summary, changes, optional") |
...
**Superseded 2026-07-27:** the owner's wording ruling (commit `406e91b`) replaced "invalid until filled, announced by validation" with "incomplete until filled, announced by a validation warning" and expanded the zero-rule clause with its 4.5 consequences. The quoted block above is the wording this document mandated and Task 4 was graded against; the shipped spec text is authoritative.
